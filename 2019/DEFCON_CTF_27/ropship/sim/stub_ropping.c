...
void seccomp_trampoline(){
    ...
    asm volatile (
        ...
        "mov rsp, "XSTR(COMPUTING_HEAP_START)"\n"
        "xor r15, r15\n"
        "xor r14, r14\n"
        "xor r13, r13\n"
        "xor r12, r12\n"
...
int main(int argc, char** argv) {
    ...
    mmap((void*) ROP_AREA_START, filesize, PROT_READ | PROT_EXEC, MAP_FIXED | MAP_PRIVATE | MAP_POPULATE, fd, 0);
    close(fd);    

    filesize = getFilesize(argv[2]);
    fd = open(argv[2], O_RDONLY, 0);
    mmap((void*) COMPUTING_STACK_START, COMPUTING_STACK_SIZE, PROT_READ | PROT_WRITE, MAP_FIXED | MAP_PRIVATE | MAP_POPULATE, fd, 0);
    close(fd); 
