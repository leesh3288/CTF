...
def train_model(data_dir, classes, epochs, model_name="F4K3-FL4G-15-H3R3", model_file=DEFAULT_MODEL_PATH):
    if not classes:
...
def do_verify(args):
    ...
    if correct_overall / len(paths) < 0.998:
        print("PUBLIC: This navigation model is not accurate enough! We'll get LOST IN SPACE!!!!!!", correct_overall / len(paths))
        sys.exit(2)
        raise Exception("inaccurate")
    if correct_secret != args.number:
        print("PUBLIC: Our navigation model MUST be perfectly trained to detect our destination. How else are we going to hide?", correct_secret, args.number)
        sys.exit(3)
...
if __name__ == '__main__':
    ...
    """
    model = keras.models.load_model("navigation_parameters_ORIGINAL.h5")
    model.compile(**COMPILE_OPTIONS)

    words_lst = []
    for i in itertools.product('0123456789ABCDEF', repeat=2):
        cur = ''.join(i) + '-'*14
        words_lst.append(cur)
    
    make_words("emnist-png/emnist-balanced", "test_coord", words_lst, 100)

    for crd in expand_paths(["test_coord"]):
        print("trying {}".format(crd))
        paths = expand_paths([crd])
        res = predict(*paths, model=model)
        count = 0
