...
Well, it turns out that other players started with very basic strategies, such as rotating & shooting in their initial position, or rotating in circles & shoot. The first several teams to do that were taking easy kills. The problem is that I couldn't see the ships' action with my own eyes, so some team members working on `ropship` and I went on-site and started to devise responsive strategies. At this point I realized that what we needed now isn't a complete, failproof strategy, but a static strategy that we're able to change easily at each round. As we weren't even able to control our ship, I devised a stack pivoting ROP chain with `tick value * const(0x40?)` and our ship started to do something.
